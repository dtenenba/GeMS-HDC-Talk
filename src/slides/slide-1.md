...
<img src="img/logging.png"/> <br/>Viewing log entries in the CloudWatch console.
...
* In our case, the Flask application has  RESTful endpoints (to be consumed by workers
  and interested clients such as webapps). This was developed using the
  [Flask-RESTful](https://flask-restful.readthedocs.io/en/0.3.5/) package.
...
* Numerous [event sources](https://docs.aws.amazon.com/lambda/latest/dg/invoking-lambda-function.html) are supported. There is scope to automate more parts of the pipeline using
...
--- | --- | --- | ---
`/job` | `POST` |  Yes | Start a new job. | SciScloud web app
...
`/stop_instances` | `POST` | Yes | Terminate instances associated with a job. Only used by the final worker on a job. | Workers
...
[
    {
        ...
        "session_key": "direct.typing-2016-10-27-PT140-class-2-161213-22:58:9397645206",
        "workers": [
            {
...
* Since there are two separate codebases (the Flask web app
  running as a Lambda function and the code that runs on
  the workers), and only one of them
...
"app store" and data repository provided by Illumina (the maker of the sequencer
...
* Service was not reliable (downtime). No other
  way to start a job.
...
* Continuous Integration (CI) for Java code with [CircleCI](https://circleci.com/). ([Travis](https://travis-ci.org/) is also very nice, and widely adopted, but CircleCI is smart enough to figure out how to build most projects without being told). Use of CI could be expanded to lint/test Python code.
